...
from deployment_helpers.aws.elastic_beanstalk import (
    allow_443_traffic_from_load_balancer_to_instances, check_if_eb_environment_exists,
...
from deployment_helpers.constants import (APT_MANAGER_INSTALLS, APT_SINGLE_SERVER_AMI_INSTALLS,
    ...
    DO_SETUP_EB_UPDATE_OPEN, DO_UPDATE_OLD_ENVIRONMENT_443, ENVIRONMENT_NAME_RESTRICTIONS,
    EXTANT_ENVIRONMENT_PROMPT, FILES_TO_PUSH_EARLY, FILES_TO_PUSH_LATE, FILES_TO_PUSH_ROOT,
...
def do_update_old_environment_to_allow_last_leg_encryption():
    print(DO_UPDATE_OLD_ENVIRONMENT_443)
    existing_name = prompt_for_extant_eb_environment_name()
    do_fail_if_bad_environment_name(existing_name)
    validate_beiwe_environment_config(existing_name)  # Exits if any non-autogenerated credentials are bad.
    allow_443_traffic_from_load_balancer_to_instances(existing_name)
...
def cli_args_validation():
    ...
    parser.add_argument('-update-old-environment-last-leg-encryption', action="count", help=DO_UPDATE_OLD_ENVIRONMENT_443)
...
if __name__ == "__main__":
    ...
    if arguments.update_old_environment_last_leg_encryption:
        do_update_old_environment_to_allow_last_leg_encryption()
        EXIT(0)
