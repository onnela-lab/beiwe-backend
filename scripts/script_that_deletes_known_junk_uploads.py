...
from constants.data_stream_constants import SURVEY_ANSWERS
from database.models import ChunkRegistry, S3File
...
# acbd957a28d8833567be73376a20e27bcffa1b19 -  survey answers b'question id,question type,question text,question answer options,answer'  # survey answers
...
# 7dbf867f2f35693d69a4bf0c36ffb21f8e13f81a - b'timestamp,question id,question type,question text,question answer options,answer'   # survey timings
...
# it turns out that survey answers can exist as just the header line, but only on android and older ios.
# timings shouldn't have this problem because it is deduplicated and normalized, but it existed
# on 12 files from 2019 - which is probably a since-fixed old bug - but these have a small number of 
# files so it should be fine to disable deleting them.  teeny tiny storage cost.
sha1_hashes = [
    # "21f38aedd23cbb507f185303cc914880fb62ec6f", survey answers
    # "acbd957a28d8833567be73376a20e27bcffa1b19", survey answers
    # "2701ea94a0a8578b480cd69903b958aaaa2ad369", survey timings
    # "2cb0183eab9e8bfcd99c4e9ac8ee304cd912eb0c", survey timings
    # "3532fbaea0361e2e26f5d5023c95398520fdc33c", survey timings
    # "47f1e17a51a5a4bbf9cadb4fe7829645745b931c", survey timings
    # "7dbf867f2f35693d69a4bf0c36ffb21f8e13f81a", survey timings
    # "8f111b9d55e88f062bbd1e18f959bad93ea681ba", survey timings
    # "a76efa6915a7545e5176a33beb770b3068c603c9", survey timings
    # "d7c0170ea85bb05fb961195259209528e1cb4e8b", survey timings
...
def main():
    ...
    for a_hex in sha1_hashes:
        ...
        while paths:= list(S3File.flat("path", sha1=bytes.fromhex(a_hex), created_on__lt=one_week_ago)[:10_000]):
            ...
            # turns out we sometimes detect files that were just uploaded, those need to go through
            # processing first.  Filtered by creation time of one week, long but whatever.
